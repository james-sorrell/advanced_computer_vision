...
def load_img_and_preprocess(path, shape=None):
  img = keras.preprocessing.image.load_img(path, target_size=shape)
...
def transferStyle(content_path, style_path):
  ...
  content_model = keras.models.Model(vgg.input, vgg.layers[13].get_output_at(0))
...
def main():
    ...
    parser.add_argument('content', type=str, help='path to model')
    parser.add_argument('style', type=str, help='path to model')
    options = parser.parse_args()
    image = transferStyle(options.content, options.style)
