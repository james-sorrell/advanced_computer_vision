...
def VGG16_AvgPool(shape):
  ...
  vgg = keras.applications.vgg16.VGG16(input_shape=shape, weights='imagenet', include_top=False)

  new_model = keras.models.Sequential()
  for layer in vgg.layers:
    if layer.__class__ == keras.layers.MaxPooling2D:
...
def VGG16_AvgPool_CutOff(shape, num_convs):
  ...
  for layer in model.layers:
    if layer.__class__ == keras.layers.Conv2D:
      n += 1
    new_model.add(layer)
    if n >= num_convs:
      break
...
def unpreprocess(img):
  # undoes preprocess input from keras
  img[..., 0] += 103.939
  img[..., 1] += 116.779
  img[..., 2] += 126.68
  img = img[..., ::-1]
...
if __name__ == '__main__':
  ...
  content_model = VGG16_AvgPool_CutOff(shape, 8)
